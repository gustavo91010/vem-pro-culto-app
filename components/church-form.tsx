...
export function ChurchForm({ church, onSave, onCancel }: ChurchFormProps) {
  ...
  const [cep, setCep] = useState(church?.endereco?.split(",")[2]?.trim() || "")
  const [logradouro, setLogradouro] = useState(church?.endereco?.split(",")[0] || "")
  const [numero, setNumero] = useState("")
  const [city, setCity] = useState(church?.cidade || "")
  const [neighborhood, setNeighborhood] = useState(church?.bairro || "")
  const [phone, setPhone] = useState(church?.telefone || "")
  const [website, setWebsite] = useState(church?.site || "")
